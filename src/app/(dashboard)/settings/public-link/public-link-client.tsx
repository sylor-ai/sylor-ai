  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (!cancel) setErr("Could not load settings.");
          return;
        }
        if (!cancel) {
          setSlug(res.publicSlug ?? "");
          setEnabled(res.publicCaptureEnabled ?? false);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0b] text-white flex items-center justify-center px-4">
        <div className="text-sm text-white/70">Loading public link...</div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#0a0a0b] text-white flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-xl rounded-[18px] border border-white/10 bg-[#0f1011]/70 p-6 shadow-xl">
        <div className="mb-4">
          <p className="text-xs text-white/40 mb-1">Settings</p>
          <h1 className="text-2xl font-semibold">Public lead link</h1>
          <p className="text-sm text-white/55 mt-1">
            Share a simple link where homeowners can request a quote. Sylor AI will text them from your Twilio
            number once they submit.
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] text-white/30">
              Letters, numbers and dashes only. This must be unique across all Sylor AI customers.
            </p>
          </div>
          ...
          {fullUrl ? (
            <div className="rounded-[12px] bg-white/5 border border-white/10 px-3 py-2 flex items-center justify-between gap-2">
              <div className="text-xs break-all">
                <span className="text-white/50">Your link:</span> <span className="text-white">{fullUrl}</span>
              </div>
              <button
                type="button"
            ...
            <p className="text-[11px] text-white/30">Once enabled, you'll see your shareable link here.</p>
          )}
        ...
        <p className="mt-4 text-[11px] text-white/30">
          When someone fills out this form, Sylor AI creates a lead, starts a conversation and sends them a welcome
          SMS. Replies go straight into your Messages inbox.
        </p>
      </div>
    </div>
  );
